...
/**
 * Walk crust that is lying double over to the hole next to it.
 *
 * Two failures, measured in the same units, turn out to be two halves of one.
 * Across every run the bare share and the doubled share track each other
 * almost exactly -- 0.45% against 0.43%, 0.17% against 0.23%, 2.06% against
 * 2.16% -- which is not a shell with too much crust on it. It is a shell with
 * about the right amount, wrinkled: a fold here and a hole a few triangles
 * away. Measured on the run this was written for, the distance from a doubled
 * direction to the nearest bare one has its median at 154 km at 120 Ma and
 * its tenth percentile at 48, against a triangle 42 km across.
 *
 * So the pile is walked into the hole. This is the slide that was tried and
 * failed before it -- the weaker coverer pushed out through its shallowest
 * edge -- with the one thing that attempt was missing: somewhere to go. That
 * version chose its direction from the overlap's own geometry, which in an
 * over-packed patch points at the next overlap, and it measured worse (0.23%
 * to 0.39% at 40 Ma, worse again with more rounds). This one chooses the
 * direction from where the crust is *missing*, so the same motion clears the
 * overlap and closes the hole.
 *
 * A fraction of the way per round, not the whole way, because the springs and
 * the area constraint have an opinion about the crust in between and this is
 * a nudge rather than a teleport. What has no hole within `reachKm` is left
 * to `unstack`, which is still the right answer for a pile with nowhere to be.
 */
export function drain(
  ...
  doubledAt: number[],
  /** The bare probe indices from the same pass. */
  bare: number[],
  probes: Float64Array,
  reachKm: number,
  radiusKm: number,
  strength: number,
  /** Accumulated displacement per vertex, reused between calls. */
  target: Float64Array,
  weight: Float64Array,
  /** Bare probes per grid cell, reused between calls. */
  buckets: number[][],
): number {
  for (const list of buckets) list.length = 0
  for (const p of bare) {
    buckets[cellOf(probes[p * 3], probes[p * 3 + 1], probes[p * 3 + 2])].push(p)
  }
  target.fill(0)
  weight.fill(0)
  const reach = Math.cos(Math.min(Math.PI, reachKm / radiusKm))
  // How many rings of cells that reach is worth, so a search does not walk the
  // whole grid to conclude there is nothing near.
  const rings = Math.max(1, Math.ceil(reachKm / radiusKm / (Math.PI / GRID_ROWS)))
  let walked = 0
  for (let i = 0; i < doubledAt.length; i += 3) {
    const p = doubledAt[i]
    const f = doubledAt[i + 1]
    const g = doubledAt[i + 2]
    // A suture, not a softness: two cratons in one place are left to be
    // counted rather than shoved about. Same rule as `unstack`.
    ...
    for (let dr = -rings; dr <= rings; dr++) {
      ...
      for (let dc = -rings; dc <= rings; dc++) {
        const c = ((col + dc) % GRID_COLS + GRID_COLS) % GRID_COLS
        for (const q of buckets[r * GRID_COLS + c]) {
          const dot = dx * probes[q * 3] + dy * probes[q * 3 + 1] + dz * probes[q * 3 + 2]
    ...
    // The way to the hole, with the part along the probe taken out so the
    // push is a step across the sphere and not into it.
    let wx = probes[hole * 3] - dx
    let wy = probes[hole * 3 + 1] - dy
    let wz = probes[hole * 3 + 2] - dz
    const along = wx * dx + wy * dy + wz * dz
    wx -= along * dx; wy -= along * dy; wz -= along * dz
    const length = Math.sqrt(wx * wx + wy * wy + wz * wz)
    if (length < 1e-12) continue
    // The whole way is the angle between the two directions; a round takes a
    // fraction of it and the next round measures again.
    const step = Math.acos(Math.min(1, best)) / length
    const weak = rigidity[f] <= rigidity[g] ? f : g
    for (let k = 0; k < 3; k++) {
      const j = faceVerts[weak * 3 + k] * 3
      target[j] += wx * step; target[j + 1] += wy * step; target[j + 2] += wz * step
      weight[j / 3] += 1
    }
    walked++
  }
  ...
  for (let v = 0; v < vertexCount; v++) {
    ...
    const i = v * 3
    const l = Math.sqrt(pos[i] ** 2 + pos[i + 1] ** 2 + pos[i + 2] ** 2) || 1
    const share = strength / weight[v]
    let nx = pos[i] / l + share * target[i]
    let ny = pos[i + 1] / l + share * target[i + 1]
    let nz = pos[i + 2] / l + share * target[i + 2]
    const nl = Math.sqrt(nx * nx + ny * ny + nz * nz)
    if (nl < 1e-9) continue
    // Its own radius kept, as everywhere else here.
    pos[i] = (nx / nl) * l; pos[i + 1] = (ny / nl) * l; pos[i + 2] = (nz / nl) * l
  }
  return walked
}
