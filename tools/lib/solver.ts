 ...
 *
 * **Off by default, and that is a decision rather than a discovery.** A reader
 * set a hard requirement -- zero gaps and zero overlap, and every force,
 * calculation and check yields to it -- and there is exactly one setting of
 * this model that meets it. Collapsing the un-erupted crust out of the mesh
 * keeps the live crust a *closed triangulation of the sphere*, and a closed
 * triangulation covers every direction once, whatever shape its triangles are.
 * So the requirement is met by construction and not by tuning:
 *
 * |  | fold | collapse |
 * |---|---|---|
 ...
 * | past its deformation budget | x76 | **x9** |
 *
 * The budget falling sevenfold is the same fact said twice: a closed mesh
 * cannot hide a mismatch in a hole, so the crust has to carry it, and once it
 * has to carry it the solver stops being able to pretend.
 *
 * What this costs is the old end of the run, and it costs it plainly. At 100 Ma
 * the held-back pairs go from 330 km to 694, and at 120 from 380 to 676; the
 * young end improves instead, 189 km to 158 at 40 Ma and 68% to 81% within 200
 * km at 20. The fold was adopted for the old end and it did buy it -- at the
 * price of a tenth of the sphere being nothing at all, which is not a price
 * this model should have been paying. Closing the Atlantic on a globe with
 * holes in it is not closing it.
 *
 * `FOLD_IN=1` puts the fold back, exactly, to the byte. The trade is written up
 * in MODEL.md under *Zero gaps first, and what it costs*.
 *
 ...
 * not what ships.
 */
const foldsInward = () => Number(ENV.FOLD_IN ?? 0) > 0
