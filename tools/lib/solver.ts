...
const contactKm = () => Number(ENV.CONTACT_KM ?? 200)
let CONTACT_KM = contactKm()
 ...
 *
 * **On by default, after a round trip through turning it off.** A reader set a
 * hard requirement -- zero gaps and zero overlap, everything else yields --
 * and the collapse meets it by construction, because the live crust stays a
 * closed triangulation of the sphere and a closed triangulation covers every
 * direction once. It also merges vertices as it goes: 40,962 points become
 * 23,391 by 90 Ma, so triangles grow to thousands of kilometres and the surface
 * map is smeared across them. The same reader called those the grey patches,
 * and they are the collapse deleting the crust's own resolution.
 *
 * The fold deletes nothing. Un-erupted crust is pushed inside the shell where
 * it carries no force, keeps its corners and its name, and can be brought back
 * out -- which is what lets this model claim the crust is conserved rather than
 * merely accounted for, and what stops a ridge being drawn where it ought to
 * have vanished. It is the more faithful reading of the hypothesis and the
 * reader wants it kept.
 *
 * So the requirement has to be met *with* the fold, by making the ridge slot
 * actually shut rather than by deleting the crust around it. See
 * CONFIG.shutSlack for the crust beside the rim giving up its resistance, and
 * CONFIG.weldRim for the rim being clapped together as a projection instead of
 * asked nicely by a spring. What that gets to is written up in MODEL.md under
 * *Zero gaps first, and what it costs*.
 *
 * `FOLD_IN=0` restores the collapse exactly, to the byte, and its run is
 * published beside this one.
 *
 ...
 * not what ships.
 */
const foldsInward = () => Number(ENV.FOLD_IN ?? 1) > 0
let FOLDING = foldsInward()
...
function readConfig() {
  ...
  /**
   * How hard the rim of the curtain is clapped shut, as a projection rather
   * than a spring.
   *
   * A reader asked the obvious question: *can you not just let those edges
   * clap against each other?* The rim has always been a spring -- one Newton
   * step per sweep at full stiffness towards zero separation -- and a spring
   * competing with the area constraint, the edge springs, the sphere and the
   * fold does not win. Measured, it does not win: the rim's median edge sits 50
   * to 66 km apart at every step of the shipped run, and the crust between
   * those corners does not exist, so those tens of kilometres *are* the hole.
   *
   * This is the same statement made as a projection. After the sweeps, every
   * closing face has its three corners moved onto their common direction --
   * each keeping its own radius, so the fold's business of how deep a corner
   * sits is untouched -- and it is done as one simultaneous pass so that no
   * face's turn in the loop matters. A triangle of crust that does not exist
   * yet is then a triangle of no width on the surface, which is what it should
   * have been all along, and the live crust either side has to have stretched
   * over what it vacated.
   *
   ...
   *
   * The requirement is zero gaps, and three attempts to reach it by working on
   * the *ridge* all failed the same way. Softening what the rim competes with
   * took the bare sphere from 10.4% to 9.5%. Welding the rim as a projection
   * took it to 1.7% at 40 Ma and stopped, because a curtain of un-erupted crust
   * is a patch and zipping a patch's edges is smoothing -- its interior corners
   * sit symmetrically between their neighbours and do not move.
   *
   * So this works on the hole instead. Coverage already walks a hundred
   * thousand probe directions and knows exactly which have nothing over them;
   * this hauls the nearest *live* crust to each of them. Not the curtain's own
   * corners -- dragging those over the sky is drawing a ridge where the ridge
  ...
  weldRounds: Number(ENV.SHUT_WELD_ROUNDS ?? 6),
  /**
...
export function solve(): void {
  ...
  /** Accumulators for weldRim and fillSky, which average before they move anything. */
  const weldTarget = new Float64Array(vertexCount * 3)
   ...
   */
  /**
   * How far apart each external conjugate pair still is, while it still is a
   * pair.
   *
   * The two sides stop being two points the moment the crust between them dies
   * and the mesh collapses them together, and after that the distance between
   * them is zero because they are one place. Zero is the right answer -- they
   * were one place at the ridge -- but it says nothing about how far the model
   * had to haul them to get there, and that distance is the closure error. So
   * it is kept, frame by frame, until the frame it disappears.
   */
  const externalLast = new Float64Array((meta.externalPairs ?? []).length).fill(-1)
  const recordExternalSeparation = (t: number) => {
    const pairs = meta.externalPairs ?? []
    for (let i = 0; i < pairs.length; i++) {
      const a = mesh.survivor(pairs[i].aVert)
      const b = mesh.survivor(pairs[i].bVert)
      if (a === b) continue
      const ia = a * 3
      const ib = b * 3
      const la = length3(pos[ia], pos[ia + 1], pos[ia + 2]) || 1
      const lb = length3(pos[ib], pos[ib + 1], pos[ib + 2]) || 1
      const dot = Math.min(1, Math.max(-1,
        (pos[ia] * pos[ib] + pos[ia + 1] * pos[ib + 1] + pos[ia + 2] * pos[ib + 2]) / (la * lb)))
      externalLast[i] = Math.acos(dot) * radiusAt(t)
    }
  }
  ...
  for (let t = CONFIG.stepMa; t <= endTimeMa; t += CONFIG.stepMa) {
    ...
    for (let sweep = 0; sweep < CONFIG.sweeps; sweep++) {
      ...
      if (CONFIG.areaHold > 0) {
        holdArea(
          ...
          // The crust beside a ridge that has to shut gives up its hold on its
          // own *area* as well as on its edge lengths. Softening only the
          // springs was half the job and it showed: the area constraint is the
          // one that actually refuses to stretch, and leaving it at full
     ...
     *
     * Measured against the hole itself rather than against the ridge, because
     * the ridge is a patch and could not be zipped shut. Each round asks
     * coverage where the sky is bare, hauls the nearest live crust there, and
     * re-imposes the sphere and the fold.
     *
     * **Last thing in the step, after the retriangulation.** Run before it,
     * this got the bare sky down to 11 to 45 probes of a hundred thousand --
     * 0.03% -- and the frame still recorded 0.88%, because `settleCollapsed`
     * and `retriangulate` come next and redraw the mesh over the holes it had
  ...
  const external = (meta.externalPairs ?? []).map((pair) => {
    ...
    // Both sides welded into one point. Zero is where the model put them and
    // it is the right target -- they were one place at the ridge -- so that is
    // what `separationKm` says. What it cannot say is how far they had to be
    // hauled to get there, so that is carried beside it. See
    // Meta.externalPairs.weldedFromKm.
    if (mesh.survivor(pair.aVert) === mesh.survivor(pair.bVert)) {
      const from = externalLast[meta.externalPairs.indexOf(pair)]
      return {
        ...pair,
        separationKm: 0,
        ...(from >= 0 ? { weldedFromKm: Math.round(from) } : {}),
      }
    }
  ...
  if (external.length) {
    ...
    const welded = external.filter((e) => e.weldedFromKm !== undefined)
    const open = external.filter((e) => e.weldedFromKm === undefined)
    const quantileOf = (xs: number[], p: number) =>
      [...xs].sort((a, b) => a - b)[Math.floor(p * (xs.length - 1))] ?? 0
    if (welded.length) {
      const from = welded.map((e) => e.weldedFromKm ?? 0)
      console.log(
        `  ${welded.length} of ${external.length} segments are shut, welded into one point `
        + `by the collapse -- which is the right answer, since they were one place at the `
        + `ridge. How far they were hauled to get there: median `
        + `${quantileOf(from, 0.5).toFixed(0)} km, p90 ${quantileOf(from, 0.9).toFixed(0)}, `
        + `${from.filter((x) => x <= 25).length} of them inside the 25 km a published fit `
        + 'reaches at 83 Ma',
      )
      console.log(`  the other ${open.length} are still open, and read:`)
    }
    const byPair = new Map<string, typeof external>()
    for (const e of (welded.length ? open : external)) {
    ...
    const all = (welded.length ? open : external).map((e) => e.separationKm)
    console.log(
...
/**
 * Clap the rim of the curtain shut: every closing triangle to no width.
 *
 * The crust between a rim's three corners does not exist yet, so they belong at
 * one place on the sphere. A spring has never got them there -- the median rim
 * edge stays 50 to 66 km open at every step -- because it is one Newton step a
 * sweep against an area constraint and edge springs that both refuse the
 * stretch it would take. So this stops asking and moves them.
 *
 * Simultaneous, not in place. A corner can belong to several rim edges and to
 * live crust besides, so every target is accumulated and averaged before
 * anything moves; edge by edge, the answer would depend on the order the edges
 * happen to be in.
 *
 * Each corner keeps its own radius exactly. Where on the sphere it belongs is
 * this pass's business; how deep it hangs is the fold's, and they must not be
 * conflated -- an earlier version of the rim spring did conflate them and spent
 * half the closure pulling the shore inward for `relaxToSphere` to undo.
 *
 * The live crust either side is dragged in by its shared corners, which is the
 * point: the hole gets filled by the crust that exists stretching over it, and
 * the area budget says the crust that exists has the area to within three parts
 * in a thousand.
 */
function weldRim(
  pos: Float64Array,
  ...
  vertexCount: number,
  strength: number,
  /** Accumulators, reused across rounds and steps. */
  target: Float64Array,
  weight: Float64Array,
): number {
  target.fill(0)
  ...
  for (let f = 0; f < faceCount; f++) {
    if (!closing[f]) continue
    welded++
    // Edge by edge, not face by face. Pulling each corner towards the centroid
    // of its own triangle works while the curtain is one triangle wide and
    // stops working as it grows: a corner in the middle of a wide curtain
    // belongs to many rim faces, the mean of their centroids is roughly where
    // it already sits, and the weld blurs into nothing. Measured -- at 1 Ma it
    // took the median rim edge from 42 km to 19, and by 10 Ma from 44 to 42.
    //
    // A rim edge has one thing to say and it is local: my two ends belong at
    // one place. Zipping each edge towards its own midpoint pulls a corner half
    // an edge, however wide the curtain is, and iterating draws the whole
    // curtain onto the line its ridge ran along -- which is where a curtain of
    // crust that has not erupted should have no width at all.
    for (let k = 0; k < 3; k++) {
      const u = faceVerts[f * 3 + k]
      const v = faceVerts[f * 3 + ((k + 1) % 3)]
      const i = u * 3
      const j = v * 3
      const la = length3(pos[i], pos[i + 1], pos[i + 2]) || 1
      const lb = length3(pos[j], pos[j + 1], pos[j + 2]) || 1
      let mx = pos[i] / la + pos[j] / lb
      let my = pos[i + 1] / la + pos[j + 1] / lb
      let mz = pos[i + 2] / la + pos[j + 2] / lb
      const ml = length3(mx, my, mz)
      // Two ends at opposite poles have no midpoint; leave them to the sweeps.
      if (ml < 1e-9) continue
      mx /= ml; my /= ml; mz /= ml
      target[i] += mx; target[i + 1] += my; target[i + 2] += mz
      weight[u] += 1
  ...
  for (let v = 0; v < vertexCount; v++) {
    ...
    const i = v * 3
    const tl = length3(target[i], target[i + 1], target[i + 2])
    if (tl < 1e-9) continue
    const l = length3(pos[i], pos[i + 1], pos[i + 2]) || 1
    ...
    if (nl < 1e-9) continue
    nx /= nl; ny /= nl; nz /= nl
    pos[i] = nx * l; pos[i + 1] = ny * l; pos[i + 2] = nz * l
  }
  return welded
}
...
function holdArea(
  ...
  r: number,
  stiffness: number,
  /** How far each face is from a ridge that has to shut, or undefined for no slack. */
  rimRing?: Uint8Array,
  slack = 1,
  rings = 6,
) {
